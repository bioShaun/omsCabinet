...
clean_enrich_table <- function(enrich_file) {
  ...
  if (stringr::str_detect(enrich_file_name, 'go.enrichment')) {
    enrich_df <- enrich_df[, c('numInCat', 'qvalue', 'term', 'ontology')]
    ylab_title <- '-log10(qvalue)'
    
  } else if (stringr::str_detect(enrich_file_name, 'kegg.enrichment')) {
    enrich_df <- enrich_df[, c('Input.number', 'Corrected.P.Value', 'X.Term', 'Database')]
    colnames(enrich_df) <- c('numInCat', 'qvalue', 'term', 'ontology')
...
out_prefix <- str_replace(enrich_file, 'enrichment.txt', 'enrichment.scatter_plot')
enrich_data_list <- clean_enrich_table(enrich_file)
enrich.table <- enrich_data_list$table
enrich.table$log_pval <- -log10(enrich.table$qvalue)
