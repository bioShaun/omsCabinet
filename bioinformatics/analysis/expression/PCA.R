...
write.table(PCA_data, file=paste(argv$out_path, 'PCA_data.txt', sep='/'), sep='\t', quote=F)
# prepare color
onmath_color <- c("#386cb0", "#fdb462", "#7fc97f", "#ef3b2c", "#662506", "#a6cee3",
  "#fb9a99", "#984ea3", "#ffff33")
ngroup <- length(unique(group_sample$V1))
group_colors <- colorRampPalette(onmath_color)(ngroup)
...
PCA_plot <- function(PCA_data){
  p <- ggplot(PCA_data,aes(PC1,PC2))+
    ...
    theme_onmath()+scale_color_manual(values = group_colors)+
    labs(title = "PCA",
         x = paste0("PC1: ",Summary_PCA$importance[2,1]*100,"% variance"),
         y = paste0("PC2: ",Summary_PCA$importance[2,2]*100,"% variance"))
  p
}
ggsave(filename = paste(argv$out_path,"PCA_plot.pdf",sep = "/"),plot = PCA_plot(PCA_data),width = 8,height = 6)
ggsave(filename = paste(argv$out_path,"PCA_plot.png",sep = "/"),plot = PCA_plot(PCA_data),width = 8,height = 6,type='cairo',dpi = 300)
